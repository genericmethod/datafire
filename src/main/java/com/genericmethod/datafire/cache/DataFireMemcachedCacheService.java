...
public abstract class DataFireMemcachedCacheService<T extends CacheableObject> implements CacheService<T> {
  ...
  @Autowired
  private ExtendedSSMCacheManager extendedSSMCacheManager;
  ...
  public T get(CacheKey key) throws Exception {
    ...
    if (cacheObject == null) {
      log.warn("Cache Miss :: Object matching key [" + key.getCacheKey() + "] "
             + "was not found in cache.");
      return cachedObj;
    }

    cachedObj = (T) cacheObject.get();
    log.info("Cache Hit :: Object with id [" + cachedObj.getCacheId() + "] "
            + "loaded from cache with key [" + key.getCacheKey() + "]");
  ...
  private KeyIteratorImpl getAllCacheKeys() {
    ...
    try {
      ...
      log.debug("Retrieving list of keys in cache [" + getFeedCacheName() + "] on "
              + "[" + memcachedIpAddress + ":" + memcachedPort + "]");
      MemcachedClient client = (MemcachedClient) getFeedCacheFactory().getCache().getNativeClient();
      keyIterator = (KeyIteratorImpl) client.getKeyIterator(
              new InetSocketAddress(memcachedIpAddress, memcachedPort));

    } catch (Exception exception) {
      log.error("An error occurred while retrieving list of cache keys from ["
              + getFeedCacheName() + "] on ip [" + memcachedIpAddress + "] "
              + "and port [" + memcachedPort + "]", exception);
    }
